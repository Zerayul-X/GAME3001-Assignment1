	...
	if (CollisionManager::squaredDistance(P1, P2) < (halfHeights * halfHeights)) {
		if (!object2->getIsColliding()) {
			...
			case TARGET:
				...
				TheSoundManager::Instance()->playSound("power", 0);
				break;
			case OBSTACLE:
				std::cout << "Collision with Obstacle!" << std::endl;
				TheSoundManager::Instance()->playSound("bump", 0);
				break;
			default:
...
bool CollisionManager::lineAABBCheck(ship* object1, GameObject* object2)
{
	...
	glm::vec2 lineEnd = object1->getPosition() + object1->getCurrentDirection() * 50.0f;
	// aabb
...
bool CollisionManager::lineAABBCheckLeft(ship* object1, GameObject* object2)
{
	glm::vec2 lineStart = object1->getPosition();
	glm::vec2 lineEnd = object1->getPosition() + object1->getCurrentDirection() * 50.0f;
	// aabb
	...
	if (lineRectCheck(lineStart - 20.0f, lineEnd - 5.0f, boxStart, boxWidth, boxHeight))
	{
		switch (object2->getType()) {
		case OBSTACLE:
			//TheSoundManager::Instance()->playSound("bump", 0);
...
bool CollisionManager::lineAABBCheckRight(ship* object1, GameObject* object2)
{
	glm::vec2 lineStart = object1->getPosition();
	glm::vec2 lineEnd = object1->getPosition() + object1->getCurrentDirection() * 50.0f;
	// aabb
