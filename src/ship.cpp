...
void ship::turnRight()
{
	float angleDiff = abs(abs(angleToTarget) - m_turnRate);
...
void ship::turnAround()
{
	...
	else if (m_currentHeading > 180.0f) {
		m_currentHeading -= 180.0f;
	}
...
void ship::move()
{
	angleToTarget = Util::signedAngle(getCurrentDirection(), m_targetDirection);
	...
	setVelocity(m_currentDirection);
...
void ship::m_computeTargetDirection()
{
	...
	//float angleToTarget = acos(Util::dot(getCurrentDirection(), m_targetDirection) * Util::Deg2Rad);
	//std::cout << "target Direction: " << angleToTarget << std::endl;
}
